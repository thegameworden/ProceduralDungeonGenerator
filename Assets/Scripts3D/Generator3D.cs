...
public class Generator3D : MonoBehaviour {
    ...
    /*
     Building Rooms
    [SerializeField]
    GameObject wallPrefab; // Assign your Wall Prefab in the Inspector
    [SerializeField]
    GameObject floorPrefab; // Assign your Floor Prefab in the Inspector

    void PlaceFloorPrefab(Vector3Int location)
    {
        GameObject floorInstance = Instantiate(floorPrefab, location, Quaternion.identity);
    }
    ...
    void PlaceWallPrefab(Vector3Int location, WallOrientation orientation)
    {
        // Instantiate the wall prefab at the integer location
        GameObject wallInstance = Instantiate(wallPrefab, location, Quaternion.identity);
        Vector3 offset = Vector3.zero;

        // Adjust the base location after instantiation to shift all walls correctly
        Vector3 baseLocationAdjustment = new Vector3(-0.5f, 0.5f, -1f);

        if (orientation == WallOrientation.XAxis)
        {
            wallInstance.transform.localScale = new Vector3(0.1f, 1, 1);
            // Offset to cover the floor edge along the Z-axis
            offset = new Vector3(0, 0, 0.5f); // Half the thickness along the z-axis
        }
        else if (orientation == WallOrientation.ZAxis)
        {
            wallInstance.transform.localScale = new Vector3(1, 1, 0.1f);
            // Offset to cover the floor edge along the X-axis
            offset = new Vector3(0.5f, 0, 0); // Half the thickness along the x-axis
        }

        // Apply the offset to the wall's position
        wallInstance.transform.position += offset + baseLocationAdjustment;
    }

    void PlaceRoomWithWallsAndFloor(Vector3Int roomLocation, Vector3Int roomSize)
    {
        // Instantiate the Floor Prefabs
        for (int x = 0; x < roomSize.x; ++x)
        {
            for (int z = 0; z < roomSize.z; ++z)
            {
                PlaceFloorPrefab(roomLocation + new Vector3Int(x, 0, z));
            }
        }
        ...
        for (int y = 0; y < roomSize.y; ++y)
        {
            for (int x = 0; x < roomSize.x; ++x)
            {
                PlaceWallPrefab(roomLocation + new Vector3Int(x, y, 0), WallOrientation.ZAxis); // Front wall
                PlaceWallPrefab(roomLocation + new Vector3Int(x, y, roomSize.z - 1), WallOrientation.ZAxis); // Back wall
            }

            for (int z = 0; z < roomSize.z; ++z)
            {
                PlaceWallPrefab(roomLocation + new Vector3Int(0, y, z), WallOrientation.XAxis); // Left wall
                PlaceWallPrefab(roomLocation + new Vector3Int(roomSize.x - 1, y, z), WallOrientation.XAxis); // Right wall
            }
        }
    }
